"use client";
import { AllImages, ImageFile } from "@/datas/image";
...
export default function Gallery() {
  const [indexDrop, setIndexDrop] = useState<number | null>(null);
  ...
  const handleImageSelect = (id: number) => {
    setSelectedImages((prevImages: any) => {
      if (prevImages.includes(id)) {
        return prevImages.filter((img: any) => img !== id);
      } else {
  ...
  const isSelected = (imageId: number) => selectedImages.includes(imageId);
  ...
  useEffect(() => {
    uploadImage.forEach((item: any, index: any) => {
      setImages((prev: any) => [
        ...prev,
